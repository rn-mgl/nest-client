...
const DeletePerformanceReview: React.FC<ModalInterface> = (props) => {
  const url = process.env.URL;
  const { data } = useSession({ required: true });
  const user = data?.user;

  const submitDeletePerformanceReview = async (
    ...
    try {
      ...
      if (token && user?.token) {
        ...
        if (deletedPerformanceReview.success) {
  ...
  return (
    ...
    >
      <div className="w-full h-auto max-w-(--breakpoint-t) bg-neutral-100 shadow-md rounded-lg ">
        ...
        <form
          onSubmit={(e) => submitDeletePerformanceReview(e)}
...
export default DeletePerformanceReview;
